import { lazy, LazyExoticComponent, Suspense, useEffect, useState, type ComponentType } from "react"
...
;(async () => {
    ...
    function App() {
        ...
        return (
            <div>
                {isMobile && (
                    <button className="kitchen_sink-layout-icon" onClick={() => setSidebarOpened(true)}>
                        <RxHamburgerMenu />
                    </button>
                )}
                <h1>Browser Apis Kitchen Sink</h1>
                <p>
                    Hey! We're actively recruiting cooks 🧑‍🍳 to help assemble a list of examples of your favorite Browser
                    Apis.
                </p>
                ...
                <div className="kitchen_sink-layout">
                    <div
                        className={`kitchen_sink-sidebar ${
                            sidebarOpened ? "kitchen_sink-sidebar_open" : "kitchen_sink-sidebar_close"
                        }`}
                    >
                        <div className="kitchen_sink-sidebar_container">
                            {isMobile && (
                                <button className="kitchen_sink-layout-icon" onClick={() => setSidebarOpened(false)}>
                                    <AiOutlineClose />
                                </button>
                            )}
                    ...
                    <div className="kitchen_sink-content">
                        <div className="kitchen_sink-content_container">
                            <details open={hasSelected}>
                                <summary>
                                    Currently Selected: <strong>{hasSelected ? modules[selected]![0] : "None"}</strong>
                                </summary>
